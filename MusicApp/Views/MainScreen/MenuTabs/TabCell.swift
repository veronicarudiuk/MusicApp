...
class TabCell: UICollectionViewCell {
  ...
  var indicatorView: UIView!
    var indicatorColor = UIColor(named: K.BrandColors.blueColor)
      ...
      DispatchQueue.main.async {
        UIView.animate(withDuration: 0.2) {
          self.indicatorView.backgroundColor = self.isSelected ? self.indicatorColor : UIColor.clear
            self.tabTitle.textColor = self.isSelected ? UIColor(named: K.BrandColors.blueColor) : .white
          self.layoutIfNeeded()
        }
      }
    }
...
//    tabSV.distribution = .fillEqually
//    tabSV.alignment = .center
  ...
  func setupIndicatorView() {
    ...
    NSLayoutConstraint.activate([
      ...
      indicatorView.topAnchor.constraint(equalTo: tabTitle.bottomAnchor, constant: 10)
    ])
  }
}
